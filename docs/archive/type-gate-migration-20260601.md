...
2026-06-01 完成 Pyright 阻塞门禁迁移。迁移前的 `8 errors, 11 warnings` 中，8 个 optional SDK 动态边界 error 已通过窄化 helper 修复。

迁移完成后的 Pyright 状态为 `0 errors, 11 warnings`：

- `broker/__init__.py` 的 10 个 warning 来自延迟导出列表；兼容导出仍需保留。
- `config.py` 的 1 个 warning 是 PyYAML source 可见性提示，不影响 Pyright 阻塞门禁。

任何新增 Pyright error 都会阻塞类型门禁。保留 warning 不能用于绕过执行、风控、状态或 targets contract 缺陷。
