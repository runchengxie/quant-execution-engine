...
| A 股 / CN 文件契约 | `targets.json` 可解析 `market: CN` 目标，适合作为中国大陆市场研究到执行的 dry-run 合约验收。 | 由 `cstree export-targets` 生成的 `.SH`、`.SZ`、`.BJ` 标的应保留交易所后缀；`.XSHG`、`.XSHE` 会在导出层标准化为 `.SH`、`.SZ`。 | 仅是文件契约级别能力，不代表已连接真实券商。 | 当前券商后端没有宣称中国大陆市场真实报单能力；真实账户权限、券商接口、港股通或 A 股账户能力必须单独验证。 |
