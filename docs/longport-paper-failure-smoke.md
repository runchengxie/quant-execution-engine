...
- 操作员能否先看事实，再修本地状态
- 状态维护命令范围是否足够克制，能否避免手工删 `outputs/state/*.json`
...
- `PENDING_CANCEL`：  
  先 `reconcile`，再决定后续是否需要 `retry` / `reprice`
