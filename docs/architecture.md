...
*   网关依赖模式：针对盈透证券，将其定位为依赖本地运行环境的后端，目前严格通过本地部署的 IB Gateway 配合 TWS API 进行接入，而非视作纯云端接口。
...
*   风控依赖市场数据：买卖价差、参与率以及市场冲击等风控指标均强依赖券商下发的实时行情。若无法获取有效行情，系统会如实记录为跳过校验，而不会强行捏造伪指标来阻断交易。
