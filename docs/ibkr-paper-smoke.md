...
- 明确本地运行盈透网关配合应用编程接口是先决条件；切勿将盈透证券视作纯云端接入的券商后端。
...
- 当前机器可以访问该端口。
- 当前登录的是模拟盘会话，而非实盘。
- 如果代码运行在子系统且网关运行在宿主机上，请优先尝试本地回环地址；若在网络地址转换模式下无法连接，再将环境变量修改为宿主机的地址。
...
注：输出目录默认被代码版本控制忽略，此处提到的测试证据是指留在本地供后续复查的档案，并非随代码库提交的测试用例夹具。
