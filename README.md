...
*   长桥模拟盘（`longport-paper`）已具备人工监督的模拟盘提交、查询、对账、撤单证据。
*   长桥实盘已验证只读路径和实盘保护机制，但真实下单仍按人工监督路径推进。
*   盈透模拟盘（`ibkr-paper`）依赖本地 IB Gateway，目前只支持美股正股最小切片，仍缺有效行情下的下单、撤单和成交证据。
*   `orders` / `exceptions` / `order` 是本地追踪状态视图，不是券商全量订单簿。
*   `broker-orders` / `broker-fills` 是单独的券商端只读查询命令，仅在支持的后端上可用。
*   `trace-order` 会把本地追踪状态和券商端历史组合成一个复查视图；历史记录不可用时仍可返回本地追踪记录。
*   `--account` 是标签解析与快速失败校验，不是多账户路由。
...
*   `.env.example` 仅作为本地模拟盘或冒烟测试示例，不包含长桥实盘 Token 占位符。
...
各券商的测试证据成熟度以 [docs/current-capabilities.md](docs/current-capabilities.md) 为准。
...
*   [执行底座说明](docs/execution-foundation.md)
*   [项目功能清单](docs/execution-checklist.md)
*   [测试指南](docs/testing.md)
*   专项操作手册：
    *   [Alpaca 模拟盘冒烟测试](docs/alpaca-paper-smoke.md)
    *   [盈透模拟盘冒烟测试](docs/ibkr-paper-smoke.md)
