...
def test_prefers_new_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_returns_new_when_only_new(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("LONGPORT_APP_KEY", "new")
...
def test_falls_back_to_old(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_returns_default_when_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("LONGPORT_APP_KEY", raising=False)
    monkeypatch.delenv("LONGBRIDGE_APP_KEY", raising=False)
